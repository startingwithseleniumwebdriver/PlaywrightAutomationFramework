...
public class ChromiumMobileStrategy implements BrowserLaunchStrategy {
	...
	public void launch(Playwright playwright, FrameworkConfig config) {
		...
		NewContextOptions contextOptions = new NewContextOptions().setIsMobile(true)
				.setUserAgent(config.mobileUserAgent()).setDeviceScaleFactor(config.mobileDevicePixelRatio())
				.setViewportSize(
						config.mobileLandscape() ? config.mobileViewportHeight() : config.mobileViewportWidth(),
						config.mobileLandscape() ? config.mobileViewportWidth() : config.mobileViewportHeight());
